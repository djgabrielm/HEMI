...
savepath = datadir("Trimmed_Mean", "Weighted", "SBB36","nsim125000")


sims = Vector{Dict{Symbol,Any}}()
rango_inf = LinRange(33,43,21)
rango_sup = LinRange(81,91,21)
for i in rango_inf
    for j in rango_sup
...
plot1 = heatmap(rango_inf,
            rango_sup, 
            (1 ./ MATRIX), 
            title  = L"MSE^{-1} \ para \ media \ truncada \  ponderada \ con \ SBB(36) \ y \ N_{sim} = 125,000 ",
            xlabel = L"\ell_1", 
            ylabel = L"\ell_2",
            size   = (600,400),
            titlefontsize = 10
...
cd(plotsdir("Trimmed_Mean", "Weighted", "SBB36", "nsim125000"))
